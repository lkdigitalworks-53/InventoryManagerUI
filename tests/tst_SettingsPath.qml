...
TestCase {
    ...
    function test_real_app_with_org_set_defers_to_default_resolution() {
        compare(SettingsPath.settingsLocationOverride("LKDigitalWorks", "/tmp"), "",
    ...
    function test_qmltestrunner_with_empty_org_gets_an_explicit_temp_path() {
        var result = SettingsPath.settingsLocationOverride("", "/tmp")
    ...
    function test_undefined_or_null_org_is_treated_the_same_as_empty() {
        verify(SettingsPath.settingsLocationOverride(undefined, "/tmp").length > 0)
        verify(SettingsPath.settingsLocationOverride(null, "/tmp").length > 0)
    }
    ...
    function test_the_same_shared_filename_is_returned_every_call() {
        ...
        var a = SettingsPath.settingsLocationOverride("", "/tmp")
        var b = SettingsPath.settingsLocationOverride("", "/tmp")
        compare(a, b)
    }
}
