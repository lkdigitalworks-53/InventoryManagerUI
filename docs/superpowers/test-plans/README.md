...
|---|---|---|---|
| 2026-09-01 | [`2026-09-01-loadingMore-resetPending-guard-test-plan.md`](2026-09-01-loadingMore-resetPending-guard-test-plan.md) | `feature/2026-09-01-loadingMore-resetPending-guard` | Automated + on-device. `_resetPending` fix for the account-switch-mid-sync gap Skill 39 left open, across all 6 paginated stores. Automated coverage (`tst_ResetPendingGuard.qml`, 6/6 passing) is a hand-written model, not the real singletons — `FirebaseService.query` confirmed non-injectable this session. On-device Section 3.2 (N1-N5) is the only way to confirm the real stores behave like the model; none of it has been run yet. |
