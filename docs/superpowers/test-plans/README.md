...
|---|---|---|---|
| 2026-09-14 | [`2026-09-14-order-completion-double-submit-test-plan.md`](2026-09-14-order-completion-double-submit-test-plan.md) | `fix/2026-09-14-order-completion-double-submit` | Automated + on-device, standard format (Skill 49). Approving a pending order twice before the first completion's write resolved deducted stock and recorded the sale twice (no busy indicator existed, and `LockManager`'s same-user re-grant behavior didn't catch it either). Fixed with an entity-scoped in-flight guard in `DataModel._tryCompleteOrder` plus wiring `OrderDetailDialog` up to `BottomSheet`'s existing busy-state mechanism. 4 new cases in `tests/tst_DataModel_completeOrderReentrancy.qml`, written/hand-traced, CI-pending — no automated coverage for the dialog's own busy-state UI (Felgo-dependent, on-device only). |
