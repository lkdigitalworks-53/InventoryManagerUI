...
|---|---|---|---|
| 2026-09-16 | [`2026-09-16-new-order-double-submit-test-plan.md`](2026-09-16-new-order-double-submit-test-plan.md) | `fix/2026-09-16-new-order-double-submit` | Automated + on-device, standard format (Skill 49). Same bug *pattern* as 2026-09-14 below, found via a deliberate sweep (`ASYNC-REENTRANCY-BUGS.md` C-2) rather than a user report: `NewOrderDialog.trySubmit()` had no busy guard and closed immediately after firing a fire-and-forget create signal, so a plain double-tap on "Place order" minted two orders (and double-deducted stock with auto-approve on). Fixed with the same busy+wait-for-signal shape as PR #70, plus a new dedicated `orderCreationFailed` signal (Skill 65) so the dialog isn't gated on the shared generic error bus. 9 cases in `tests/tst_NewOrderDialogSubmitGuard.qml`, written/traced this session — not yet CI-confirmed (plan written before CI ran, unlike the 2026-09-14 entry below). |
| 2026-09-14 | [`2026-09-14-order-completion-double-submit-test-plan.md`](2026-09-14-order-completion-double-submit-test-plan.md) | `fix/2026-09-14-order-completion-double-submit` | Automated + on-device, standard format (Skill 49). Approving a pending order twice before the first completion's write resolved deducted stock and recorded the sale twice (no busy indicator existed, and `LockManager`'s same-user re-grant behavior didn't catch it either). Fixed with an entity-scoped in-flight guard in `DataModel._tryCompleteOrder` plus wiring `OrderDetailDialog` up to `BottomSheet`'s existing busy-state mechanism. 6 cases in `tests/tst_DataModel_completeOrderReentrancy.qml`, genuinely run — 818/818 QML tests passing (1025/1025 overall) on PR #70, after two rounds of real CI-driven corrections (Skills 61–62: QML `function` members are read-only and can't be monkey-patched; `_tryCompleteOrder`'s happy path needs a live Gateway backend, unlike `_tryAdjustOrder`'s callback-less local-apply helpers). No automated coverage for the dialog's own busy-state UI (Felgo-dependent, on-device only). |
| 2026-09-14 | [`2026-09-14-batch-mint-retry-and-topup-safety-test-plan.md`](2026-09-14-batch-mint-retry-and-topup-safety-test-plan.md) | `fix/2026-09-14-batch-mint-retry-and-topup-safety` | Automated + on-device. Fixes the item-1 roadmap gap Taher confirmed on-device (N3): a failed batch-id mint now queues for automatic retry on reconnect instead of being silently dropped (no error shown, but the affected product is hidden from both order-entry pickers until it resolves), and `topUpOldest`'s drift-repair now always creates a new labeled "Adjustment" batch instead of ever mutating an existing batch's own history — the second, worse bug Taher's test surfaced. 11 new tests (`tst_PendingMintAndTopUpSafety.qml`), same mirror-model technique as `tst_TenantContextRaceGuard.qml` for the same reason (`addBatch`/`topUpOldest` are read-only singleton methods). Neither the real singleton nor the two dialogs' picker-filtering are exercised by any test — on-device confirmation (N1/N2/E1) is the only way to close that gap. |
| 2026-09-02 | [`2026-09-02-price-adjust-tax-delta-test-plan.md`](2026-09-02-price-adjust-tax-delta-test-plan.md) | `fix/2026-09-02-price-adjust-tax-delta` | Automated + on-device, standard format (Skill 49). Two bugs, same root cause, fixed together: a completed-order discount/price edit on a taxable line leaving the order's tax stale (with a residual after a subsequent return), and the Analysis/Reports "Tax" column silently missing the same `price_adjust` tax delta (`RealisedMath.js`, both QML and Node ports). 26 new/extended cases across 6 files. The Node side (`functions/lib/realisedMath.js`) was genuinely run this session — `node --test` 9/9 on the new cases, full `functions/` suite 195/195 — a rare case of real executed proof in a session with no Qt toolchain; the QML side is written/hand-traced, CI-pending. |
| 2026-08-30 | [`2026-08-30-product-order-delete-ui-test-plan.md`](2026-08-30-product-order-delete-ui-test-plan.md) | `feature/product-order-delete-ui` | Automated + on-device. Real CI run found and fixed a genuine pre-existing bug (`DataModel.qml` calling an undeclared `logic` — see Skill 58) and surfaced that 2 of 5 new test files can never compile under this repo's Felgo-free CI (moved to `test/felgo-dependent/`). Remaining 3 files pass on CI. On-device section leads with the conflict-wording fix (N4), the actual new failure-notification behavior, ahead of the buttons themselves. |
...
- **Order-double-submit pattern**: `2026-09-14` (completion) → `2026-09-16` (creation). Same bug
  *shape*, two different entry points, found in two separate sessions — the second via a deliberate
  sweep for the pattern rather than a new report. Read both; the 2026-09-16 plan assumes the first
  as background and doesn't re-explain the shared root-cause pattern in full.
