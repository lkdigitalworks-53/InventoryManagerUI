...
|---|---|---|---|
| 2026-09-02 | [`2026-09-02-price-adjust-tax-delta-test-plan.md`](2026-09-02-price-adjust-tax-delta-test-plan.md) | `fix/2026-09-02-price-adjust-tax-delta` | Automated + on-device, standard format (Skill 49). Fixes a completed-order discount/price edit on a taxable line leaving the order's authoritative tax stale, with a residual left after a subsequent full return. 3 new UT/functional files (14 cases total) written and hand-traced against the implementation but **not run** (no Qt toolchain this session — CI is the first real proof). Explicitly flags `RealisedMath.js`'s Analysis "Tax" column as having the same defect class, out of scope this branch. |
