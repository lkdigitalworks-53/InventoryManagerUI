...
| `functions/lib/batchMutationLogic.js` | Unchanged logic, comment only; Functions suite covers it (110 cases at the time this fix landed, +1 new pin; 175 as of the latest rebase onto `main`, after `feature/async-stock-batch-id-minting`'s, #55's, and #56's unrelated additions merged into this branch) | No behavior change — confirmed by Phase 1 root-cause investigation that the 200 cap itself is correct |
...
- Functions: `cd functions && npm test` — 110 tests, 0 failed, re-run after every change in this
  fix specifically. Rebased onto `main` twice since (`#55` handler-coverage work, then `#56`
  handler-parity work — both unrelated to this fix, merged into this branch on GitHub directly);
  re-ran fresh after each rebase to confirm: currently 175/175 passing, this fix's own tests still
  among them.
