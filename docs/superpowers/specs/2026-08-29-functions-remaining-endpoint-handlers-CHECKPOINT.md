...
## Post-hoc correction (2026-08-30, archived by the next session)

This checkpoint's "Status" and "Next action if resumed" sections above say the work described was
"not yet committed/pushed." That was true when this file was written, but the work was in fact
committed and pushed shortly after — it's `d1087b6` / PR #55 on `main`, merged before this
correction was made. Whoever merged #55 didn't update this file to say so, so it sat on `main`
claiming uncommitted work that had already landed. Archiving as-is (not silently rewriting the
stale claim) so the discrepancy stays visible — this is exactly the kind of "canonical
session-continuity document" drift the project tries to avoid, so it's worth being visible as a
finding, not just quietly fixed.
