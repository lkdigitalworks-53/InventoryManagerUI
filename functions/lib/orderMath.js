...
// Node port of qml/helper/OrderMath.js. Logic is byte-identical to the QML
// version -- only the module boilerplate differs (CommonJS require/exports
// instead of QML's `.pragma library`/`.import`, which aren't valid Node
// syntax). Parity between the two is enforced by shared JSON fixtures run
// against both tests/tst_RealisedMath.qml / tests/tst_BreakdownMath.qml (via
// the OrderMath functions they depend on) and functions/test/*.test.js --
// see functions/test/fixtures/.
//
...
// reconciles to the order. Pure -- no framework deps, unit-testable.
//
...
//   dim in "productId" | "category" | "supplier" | "supplierId"
...
module.exports = {
    ...
    lineTax: lineTax,
    allocate: allocate,
    spreadOrderDelta: spreadOrderDelta,
    spreadLineDeltaBySupplier: spreadLineDeltaBySupplier,
