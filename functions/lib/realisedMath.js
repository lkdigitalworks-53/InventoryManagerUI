...
// Node port of qml/helper/RealisedMath.js. Logic is byte-identical to the QML
// version -- only the module boilerplate differs. See functions/lib/orderMath.js
// for the parity-testing note.
//
// Single source of truth for REALISED money aggregation over the immutable
// transaction event log. Pure -- all inputs are passed in. Mirrors the
// BreakdownMath.js / OrderMath.js pattern so it is unit-testable headlessly.
//
...
//     supplierId: "" }
// lookups (functions, injected so the lib stays free of any singleton/DB dep):
...
const OrderMath = require("./orderMath");

function _round2(x) { return Math.round(x * 100) / 100; }

// Mirrors qml/helper/RealisedMath.js's _resolvedCategory exactly -- see that
// file for the full reasoning (DELETE-FEATURE-ROADMAP item 3, 2026-09-26).
...
function _passesScope(e, scope, categoryOf) {
    ...
    if (scope.category) {
        // Deliberately live-only -- see qml/helper/RealisedMath.js's matching
        // comment (DELETE-FEATURE-ROADMAP item 3): this is the FILTER, not a
        // display label, and stays out of scope for that fix.
...
//   field in "productId" | "supplierId" | "category" | "channel" | "staffId"
...
function byDimension(field, entries, scope, lookups) {
    ...
    for (var i = 0; i < entries.length; ++i) {
        ...
        // re-sum to the stamped line totals exactly (C). lineQty == Sum qtyConsumed,
...
// compute the SAME amount under a supplier filter and totals == Sum bucketWalk
...
// gated by scope and free of any singleton dep (category/order via injected lookups).
...
// Single filtered totals block. By construction equals Sum byDimension("category")
...
// section. metric in "net" | "profit". `now` is injected (tests pass a fixed
...
function bucketWalk(metric, periodIdx, entries, scope, now, lookups) {
    ...
    for (var k = 0; k < entries.length; ++k) {
        ...
        if (e.kind === "price_adjust") {
            ...
            // totals == Sum bucketWalk holds under a filter too.
...
// Sums rows that resolve to the same name and recomputes margin.
