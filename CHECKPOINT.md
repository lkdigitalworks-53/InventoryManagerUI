...
- [x] 7. Tests: 19 new/extended QML cases across 3 files (10 new `tst_StaffStore_delete.qml`, 4 new in
      ...
      `npm test` for real — 200/200 (baseline 195/195 before this branch's changes). Confirmed genuinely TDD by
      reverting the `functions/index.js` fix and rerunning (199 pass, 1 fail — exactly the new refused-role
      test), then restoring it (200/200 again).
...
- [ ] 9. Commit the docs batch, push, read CI, report the real QML result (functions result is already real).
